        ...
        if (!self.opts.staticGrid && self.opts.acceptWidgets) {
            ...
            this.dd
                ...
                .on(self.container, 'dropover', function(event, ui) {
                    ...
                    // Get data for potential new item (it can be "mini" or "previewd")
                    var width = null;
                    if (width = el.data('gs-width')) {
                      width = parseInt(width);
                    } else {
                      width = origNode ? origNode.width : (Math.ceil(el.outerWidth() / cellWidth));
                    }

                    var height = null;
                    if (height = el.data('gs-height')) {
                      height = parseInt(height);
                    } else {
                      height = origNode ? origNode.height : (Math.ceil(el.outerHeight() / cellHeight));
                    }
                ...
                .on(self.container, 'drop', function(event, ui) {
                    ...
                    // Unbind drag on cloned
                    $dragEl.unbind('drag', onDrag);

                    self.container.append(el).promise().done(function() {
                      // Custom modifications, add drop callback function
                      // callback for standart editation
                      if (_.isFunction(self.opts.dropCallback)) {
                        // Call with proper node & drag
                        self.opts.dropCallback(node, el, $dragEl);
                      }

                      // Remove gridstack node reference
                      $dragEl.data('_gridstack_node', null);
